...
check_xist_pure = function( raw_dge, rep_name, results_path ){
  ...
  data(Y_genes, package = "thymusatlastools2") 
...
#' @export
#'
save_depth_stats = function(dge_list, results_path) {
  dimsum = function(X) c(dim(X), sum(X))
  basic_stats = dge_list %>% lapply(dimsum) %>% Reduce(f = rbind) %>% as.data.frame
  colnames(basic_stats) = c("transcripts", "cells", "UMIs")
  rownames(basic_stats) = basic_stats$Sample_ID = names(dge_list)
  write.table(basic_stats, file = file.path(results_path, 
                                            "basics_stats.txt"), quote = F, row.names = F, col.names = T, 
              sep = "\t")
}
